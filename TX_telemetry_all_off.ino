...
bool low_battery_Climber = false;
bool low_battery_RC = false;
...
void loop() {
  ...
  for (int i = 0; i < 10 ; i++)
  {
    adc = adc + analogRead(ADC_Battery_LvL_Pin);
  }
  V_RC_raw = adc / 10;
  DEBUG.print("V_RC=");
  DEBUG.println(V_RC_raw);
  ...
  if (SwitchError == false && low_battery_RC == false && low_battery_Climber == false && AlarmFlag == true)
  {
    Alarm_OFF();
  }

  if ( (SwitchError == true || low_battery_RC == true || low_battery_Climber == true ) && AlarmFlag == false )
  {
    //    Alarm_ON();
  }
...
void OLED_Display_Dynamic_Data()
{
  ...
  // First line
  int S1 = map(Speed_1_raw, 50, 995, -100, 100);
  int S2 = (-1) * map(Speed_2_raw, 50, 995, -100, 100);
  simplePrint_LCD(S1, 5, 1);
  simplePrint_LCD(S2, 12, 1);

  // Second line
  int I1 = map(I_1_raw, 50, 995, -100, 100);
  int I2 = (-1) * map(I_2_raw, 50, 995, -100, 100);
  simplePrint_LCD(I1, 5, 2);
  simplePrint_LCD(I2, 12, 2);

  // Third line
  Print_RC_Battery_LvL_for_OLED(V_RC_raw, 13, 3);   // self
  Print_Climber_Battery_LvL_for_OLED(V_Roper_raw, 6, 3);
...
// byte 5 - 6 - 7  -> _-T
void Print_RC_Battery_LvL_for_OLED(int intoviy, int pos1, int pos2)
{
  if (intoviy >= 1023 || intoviy < 0)
  {
    disp.setCursor(pos1, pos2);
    disp.print("ERR");
    // alarm??
  ...
  if (intoviy >= 600 & intoviy < 642)
  {
    disp.setCursor(pos1, pos2);
    disp.write(byte(5));
    disp.print("  ");
    low_battery_RC = false;
  }

  if (intoviy >= 642 & intoviy < 705)
  {
    disp.setCursor(pos1, pos2);
    disp.write(byte(5));
    disp.write(byte(6));
...
void Print_Climber_Battery_LvL_for_OLED(int intoviy, int pos1, int pos2)
{
  if (intoviy >= 1023 || intoviy < 0)
  {
    disp.setCursor(pos1, pos2);
  ...
  if (intoviy >= 0 & intoviy < 573)
  {
    disp.setCursor(pos1, pos2);
    disp.print("LOW");
    // alarm??
    low_battery_Climber = true;
  }

  if (intoviy >= 573 & intoviy < 600)
  {
    disp.setCursor(pos1, pos2);
    disp.write(byte(5));
    disp.print("  ");
    low_battery_Climber = false;
  }

  if (intoviy >= 600 & intoviy < 627)
  {
    disp.setCursor(pos1, pos2);
    disp.write(byte(5));
    disp.write(byte(6));
...
//    LvL = 1;
  if (RSSI_LvL >= 0 & RSSI_LvL < 170)   //1
    LvL = 2;
//  if (RSSI_LvL >= 170 & RSSI_LvL < 180)
//    LvL = 3;
  if (RSSI_LvL >= 170 & RSSI_LvL < 180)  //2
    LvL = 4;
//  if (RSSI_LvL >= 190 & RSSI_LvL < 210)
//    LvL = 5;
  if (RSSI_LvL >= 180 & RSSI_LvL < 210)  //3
    LvL = 6;
//  if (RSSI_LvL >= 215 & RSSI_LvL < 217)
//    LvL = 7;
  if (RSSI_LvL >= 210 & RSSI_LvL < 255)  //4
    LvL = 8;
  return LvL;
}
